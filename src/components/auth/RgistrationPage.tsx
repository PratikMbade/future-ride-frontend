...
// "FR" + 6+ hex chars, case-insensitive — matches the backend's normalizeFutureRideId
...
// ─────────────────────────────────────────────────────────
//  ALTITUDE DIAL — the new signature element.
//  A radial instrument, not a tree: a single ring sweeps from 0 → 4
//  waypoints as the flow advances. Center readout shows the raw step
//  number until registration lands, then flips to a confirmed glyph
//  with expanding radar pulses. It's the cockpit-gauge read on the
//  same three inputs the old node-tree consumed.
// ─────────────────────────────────────────────────────────
function AltitudeDial({ step, sponsorLabel, isSuccess }: {
  ...
  const R = 78;
  const C = 2 * Math.PI * R;
  const frac = isSuccess ? 1 : (step - 1) / 4;
    ...
    <div className="relative flex flex-col items-center" aria-hidden="true">
      <div className="relative w-[176px] h-[176px]">
        <svg viewBox="0 0 176 176" className="w-full h-full -rotate-90">
          ...
          />
          {/* waypoint ticks at quarter marks */}
          {[0, 1, 2, 3].map(i => {
            const a = (i / 4) * 2 * Math.PI;
            const x = 88 + R * Math.cos(a);
            const y = 88 + R * Math.sin(a);
            const reached = i < step;
            return <circle key={i} cx={x} cy={y} r={reached ? 4.5 : 3} fill={reached ? (isSuccess ? '#22c55e' : '#38bdf8') : 'rgba(255,255,255,0.18)'} />;
          })}
        </svg>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-0.5">
          {isSuccess ? (
            ...
            <>
              ...
              <span className="font-mono text-[26px] font-bold text-white leading-none">{step}<span className="text-white/25 text-[15px]">/4</span></span>
...
// ─────────────────────────────────────────────────────────
//  WAYPOINT RAIL — horizontal markers connected by a line, replacing
//  the old progress bars. Each waypoint is a discrete, named stage —
//  so discrete dots (not continuous bars) read more honestly here.
// ─────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────
//  PANEL — bracket-cornered console card used to frame every stage.
// ─────────────────────────────────────────────────────────
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    if (!referrer)         { setStep(3); return; }
    setStep(4);
  }, [account?.address, hasSession, referrer]);
  ...
  const handleRegister = async () => {
    ...
    try {
      ...
      fetch(`${API}/api/register/fallback`, {
        ...
        // best-effort — if this fails, the event listener/sync service
        // are the remaining safety nets; nothing more to do client-side
      });
  ...
  const stages = [
    { label: 'Wallet' }, { label: 'Sign' }, { label: 'Sponsor' }, { label: 'Join' },
  ];

  // bottom action bar — what the primary CTA shows/does at each stage
  ...
  const stageTitle = ['Connect', 'Sign-in', 'Sponsor', 'Confirm'][step - 1];

  return (
    <div className="min-h-screen bg-[#040a14] flex flex-col selection:bg-[rgba(56,189,248,0.25)] selection:text-white">
      {/* ambient backdrop — instrument-panel glow, no hero gradient text */}
      ...
      <main className="flex-1 flex flex-col px-5 sm:px-8 pt-7 pb-32 max-w-[460px] w-full mx-auto">
        ...
        {/* waypoint rail up top, replacing the old text hero entirely */}
        <div className="mb-7">
          <WaypointRail stages={stages} current={step} />
        </div>

        {/* dial — the new centerpiece */}
        <div className="flex flex-col items-center mb-8">
          <AltitudeDial step={step} sponsorLabel={referrer ? referrer.futureRideId : null} isSuccess={isSuccess} />
          ...
          <p className="mt-1 mb-0 text-[13px] text-white/40 text-center max-w-[300px]">
            {isSuccess
              ...
              : 'Four checks to lock your position beneath your sponsor — fully on-chain.'}
          </p>
        </div>

        {/* ── stage content, framed in console panels ── */}
        <div className="flex flex-col gap-4">
          ...
          {step === 3 && (
            <Panel index={3} title="Sponsor">
              <p className="mt-0 mb-4 text-[13px] text-white/45 leading-relaxed">
                Enter your sponsor's referral ID to attach your node to theirs.
              </p>
              <ReferralInput
                value={referralId}
                ...
                <div className="rounded-lg border border-white/[0.08] overflow-hidden mb-3">
                  {[
                    ...
                    >
                      ...
                      <span className="text-[12.5px] font-medium text-white font-mono truncate ml-3">{value}</span>
                    </div>
                  ))}
                </div>
              )}
          ...
          {step === 4 && isSuccess && (
            <Panel index={4} title="Confirmed">
              <TxBanner state={txState} txHash={txHash} error={txError} />
            </Panel>
          )}
        </div>

        {account && hasSession && !checkingReg && !regInfo?.isRegistered && !isBusy && step < 4 && (
      ...
      {/* floating capsule CTA — inset from the edges, not full-bleed */}
  ...
  if (state === 'success') {
    return (
      <div className="flex flex-col items-center gap-3 py-4 text-center">
        <div className="relative w-[56px] h-[56px] rounded-full bg-[rgba(34,197,94,0.12)] border border-[#22c55e]/35 flex items-center justify-center" style={{ boxShadow: '0 0 30px rgba(34,197,94,0.22)' }}>
