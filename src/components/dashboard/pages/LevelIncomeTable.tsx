...
export default function LevelIncomeTable() {
  ...
  const incomeQ = useQuery<LevelIncomeResponse>({
    queryKey: ['income', 'level', address, page, pageSize, debouncedSearch, packageFilter, levelFilter],
    queryFn: async () => {
      ...
      const res = await fetch(`${API}/api/income/level?${params.toString()}`, {
        credentials: 'include',
      })
      if (!res.ok) throw new Error('Failed to load level income')
      const json: LevelIncomeResponse = await res.json()
      if (!json.success) throw new Error('Failed to load level income')
      return json
    },
  ...
  return (
    <div className="space-y-5" data-testid="level-income-page">
      <header>
        <p className="text-base font-bold text-white">Level Income</p>
        <span className="text-sm text-white/50">
          Flat per-level payouts credited from your downline's tree activity.
        </span>
      </header>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:flex-wrap">
            <div className="relative w-full sm:max-w-xs">
              ...
              <input
                data-testid="level-income-search"
                value={search}
                  ...
                  records.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 whitespace-nowrap">
                        <WalletAddress address={r.fromUserAddress} data-testid="level-income-wallet" />
                      </td>
                      <td className="px-5 py-4 whitespace-nowrap font-mono font-medium text-[#F5A623]">
                        PKG {String(r.packageNumber).padStart(2, '0')}
                      </td>
                      <td className="px-5 py-4 whitespace-nowrap">
                        <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-[#2DD4BF]/10 text-[#2DD4BF] font-mono">
                          Level {r.level}
                        </span>
                      </td>
                      ...
                      <td className="flex items-center gap-x-2 px-5 py-4 whitespace-nowrap">
                        <WalletAddress address={r.transactionHash} data-testid="level-income-transactionHash" />
