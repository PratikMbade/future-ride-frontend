...
import type { LevelIncomeResponse } from '../../../types/dashboard'
...
export default function LevelLapseIncomeTable() {
  ...
  // Numeric to match filter components (0 = all).
  ...
  const incomeQ = useQuery<LevelIncomeResponse>({
    queryKey: ['income', 'level-lapse', address, page, pageSize, debouncedSearch, packageFilter, levelFilter],
    queryFn: async () => {
      ...
      const res = await fetch(`${API}/api/income/level-lapse?${params.toString()}`, {
        credentials: 'include',
      })
      if (!res.ok) throw new Error('Failed to load level lapsed income')
      const json: LevelIncomeResponse = await res.json()
      if (!json.success) throw new Error('Failed to load level lapsed income')
      return json
    },
  ...
  return (
    <div className="space-y-5" data-testid="level-lapse-income-page">
      <header>
        <p className="text-base font-bold text-white">Level Lapsed Income</p>
        <span className="text-sm text-white/50">
          Level cuts you received because a closer upline in the chain wasn't eligible.
        </span>
      </header>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:flex-wrap">
            <div className="relative w-full sm:max-w-xs">
              ...
              <input
                data-testid="level-lapse-income-search"
                value={search}
            ...
            <table className="text-base" style={{ minWidth: 820, width: '100%' }}>
              ...
              <tbody>
                ...
                {!isLoading && records.length === 0 && (
                  <tr>
                    <td colSpan={columns.length} className="px-5 py-10 text-center text-white/60 text-base">
                      No level lapsed income records found.
                    </td>
                  </tr>
                )}
                {!isLoading &&
                  records.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 whitespace-nowrap">
                        <WalletAddress address={r.fromUserAddress} data-testid="level-lapse-income-wallet" />
                      </td>
                      <td className="px-5 py-4 whitespace-nowrap font-mono font-medium text-[#F5A623]">
                        PKG {String(r.packageNumber).padStart(2, '0')}
                      </td>
                      <td className="px-5 py-4 whitespace-nowrap">
                        <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-[#FB923C]/10 text-[#FB923C] font-mono">
                          Level {r.level}
                        </span>
                      </td>
                      ...
                      <td className="flex items-center gap-x-2 px-5 py-4 whitespace-nowrap">
                        <WalletAddress address={r.transactionHash} data-testid="level-lapse-income-transactionHash" />
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between text-sm text-white/70">
              <div className="flex items-center gap-2">
                ...
                <PageSizeDropdown
                  ...
                  testId="level-lapse-income-page-size"
                />
