...
export default function DirectIncomeTable() {
  ...
  // Numeric to match the PackageFilter component (0 = all). API layer still gets a string.
  ...
  const incomeQ = useQuery<DirectIncomeResponse>({
    queryKey: ['income', 'direct', address, page, pageSize, debouncedSearch, packageFilter],
    queryFn: async () => {
      const params = new URLSearchParams({ page: String(page), limit: String(pageSize) })
      if (debouncedSearch) params.set('search', debouncedSearch)
      if (packageFilter > 0) params.set('package', String(packageFilter))

      const res = await fetch(`${API}/api/income/direct?${params.toString()}`, {
        credentials: 'include',
      })
      if (!res.ok) throw new Error('Failed to load direct income')
      const json: DirectIncomeResponse = await res.json()
      if (!json.success) throw new Error('Failed to load direct income')
      return json
    },
  ...
  return (
    <div className="space-y-5" data-testid="direct-income-page">
      <header>
        <p className="text-base font-bold text-white">Direct Income</p>
        <span className="text-sm text-white/50">
          Payouts credited directly from members you referred.
        </span>
      </header>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:flex-wrap">
            <div className="relative w-full sm:max-w-xs">
              ...
              <input
                data-testid="direct-income-search"
                value={search}
          ...
          <div className="rounded-xl border border-white/10 overflow-x-auto">
            <table className="text-base" style={{ minWidth: 720, width: '100%' }}>
              <thead>
                  ...
                  records.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 whitespace-nowrap">
                        <WalletAddress address={r.fromUserAddress} data-testid="direct-income-wallet" />
                      </td>
