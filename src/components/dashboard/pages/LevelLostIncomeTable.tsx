...
import type { LevelLostIncomeResponse } from '../../../types/dashboard'
...
export default function LevelLostIncomeTable() {
  ...
  const incomeQ = useQuery<LevelLostIncomeResponse>({
    queryKey: ['income', 'level-lost', address, page, pageSize, debouncedSearch, packageFilter, levelFilter],
    queryFn: async () => {
      ...
      const res = await fetch(`${API}/api/income/level-lost?${params.toString()}`, {
        credentials: 'include',
      })
      if (!res.ok) throw new Error('Failed to load level lost income')
      const json: LevelLostIncomeResponse = await res.json()
      if (!json.success) throw new Error('Failed to load level lost income')
      return json
    },
  ...
  // No "redirected to" column here — LevelLostIncomeEV only carries the
  // buyer and the addresses skipped at that level, not who ended up
  // receiving the cut instead (unlike the legacy laps-based Lost Income).
  const columns = [
    { key: 'contractRegId', header: 'User Id' },
    { key: 'fromAddress', header: 'Triggered By' },
  ...
  return (
    <div className="space-y-5" data-testid="level-lost-income-page">
      <header>
        <p className="text-base font-bold text-white">Level Lost Income</p>
        <span className="text-sm text-white/50">
          Level cuts you missed because you weren't eligible at that level — redirected further up the chain.
        </span>
      </header>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:flex-wrap">
            <div className="relative w-full sm:max-w-xs">
              ...
              <input
                data-testid="level-lost-income-search"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search triggering wallet address…"
            ...
            <table className="text-base" style={{ minWidth: 820, width: '100%' }}>
              ...
              <tbody>
                ...
                {!isLoading && records.length === 0 && (
                  <tr>
                    <td colSpan={columns.length} className="px-5 py-10 text-center text-white/60 text-base">
                      No level lost income records found.
                    </td>
                  </tr>
                )}
                {!isLoading &&
                  records.map((r) => (
                    ...
                    >
                      ...
                      <td className="px-5 py-4 whitespace-nowrap">
                        <WalletAddress address={r.fromAddress} data-testid="level-lost-income-wallet" />
                      </td>
                      <td className="px-5 py-4 whitespace-nowrap font-mono font-medium text-[#F5A623]">
                        PKG {String(r.packageNumber).padStart(2, '0')}
                      </td>
                      <td className="px-5 py-4 whitespace-nowrap">
                        <span className="px-2.5 py-1 rounded-full text-xs font-bold bg-[#F87171]/10 text-[#F87171] font-mono">
                          L{r.level}
                        </span>
                      </td>
                      <td className="px-5 py-4 whitespace-nowrap font-mono font-medium text-[#F43F5E]">
                        {usd(r.amount)}
                      </td>
                      ...
                      <td className="flex items-center gap-x-2 px-5 py-4 whitespace-nowrap">
                        <WalletAddress address={r.transactionHash} data-testid="level-lost-income-transactionHash" />
            ...
            <div className="flex flex-col sm:flex-row gap-3 sm:items-center sm:justify-between text-sm text-white/70">
              <div className="flex items-center gap-2">
                ...
                <PageSizeDropdown
                  ...
                  testId="level-lost-income-page-size"
                />
