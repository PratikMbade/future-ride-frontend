...
export default function LostIncomeTable() {
  ...
  const incomeQ = useQuery<LostIncomeResponse>({
    queryKey: ['income', 'lost', address, page, pageSize, debouncedSearch, packageFilter],
    queryFn: async () => {
      const params = new URLSearchParams({ page: String(page), limit: String(pageSize) })
      if (debouncedSearch) params.set('search', debouncedSearch)
      if (packageFilter !== 'all') params.set('package', packageFilter)

      const res = await fetch(`${API}/api/income/lost?${params.toString()}`, {
        credentials: 'include',
      })
      if (!res.ok) throw new Error('Failed to load lost income')
      const json: LostIncomeResponse = await res.json()
      if (!json.success) throw new Error('Failed to load lost income')
      return json
    },
  ...
  return (
    <div className="space-y-5" data-testid="lost-income-page">

      <header>
        <p className="text-base font-bold text-white">Lost Income</p>
        <span className="text-sm text-white/50">
          Payouts that lapsed away and were redirected elsewhere — money missed, not received.
        </span>
      </header>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:flex-wrap">
            <div className="relative w-full sm:max-w-xs">
              ...
              <input
                data-testid="lost-income-search"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                placeholder="Search lapsed address…"
            ...
            <div className="relative w-full sm:w-auto">
              ...
              <select
                data-testid="lost-income-filter-package"
                value={packageFilter}
          ...
          <div className="rounded-xl border border-white/10 overflow-x-auto">
            <table className="text-base" style={{ minWidth: 920, width: '100%' }}>
              <thead>
